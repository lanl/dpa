      DOUBLE PRECISION FUNCTION STANDERROR(M,FVEC)
      IMPLICIT NONE
...
C
      ...
      STANDERROR=0.0
      DO I=1,M
         STANDERROR=STANDERROR+FVEC(I)**2
      ENDDO
      STANDERROR=DSQRT(STANDERROR/(M-1))
C
      RETURN
      END
      DOUBLE PRECISION FUNCTION ENORM(M,FVEC)
      IMPLICIT NONE
C      This is the form of the FCN routine if IOPT=1,
C      that is, if the user does not calculate the Jacobian.
      INTEGER M,I
...
C~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
C                                                                       |
C     Main Program of Nonlinear Least Square                            |
C                                                                       |
C~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
*DECK DNLS1E
      SUBROUTINE DNLS1E (IOPT, M, N, X, FVEC, TOL, NPRINT, INFO,
     +   IW, WA, LWA)
...
C
C       SUBROUTINE DNLS1E(IOPT,M,N,X,FVEC,TOL,NPRINT,
C      *                  INFO,IW,WA,LWA)
...
C       TOL = SQRT(R1MACH(4))
C C
C       CALL DNLS1E(IOPT,M,N,X,FVEC,TOL,NPRINT,
C      *            INFO,IW,WA,LWA)
...
C
      ...
      INDEX = 5*N+M
C
      CALL DNLS1(IOPT,M,N,X,FVEC,WA(INDEX+1),M,FTOL,XTOL,GTOL,
...
C
      ...
      END
*DECK DNLS1
      SUBROUTINE DNLS1 ( IOPT, M, N, X, FVEC, FJAC, LDFJAC, FTOL,
...
C 2. Subroutine and Type Statements.
C
C       SUBROUTINE DNLS1(IOPT,M,N,X,FVEC,FJAC,LDFJAC,FTOL,XTOL,
...
C       NPRINT = 0
C C
C       CALL DNLS1(IOPT,M,N,X,FVEC,FJAC,LDFJAC,FTOL,XTOL,
...
C
410      IFLAG = 1
         CALL DFDJC3(M,N,X,FVEC,FJAC,LDFJAC,IFLAG,EPSFCN,WA4)
         NFEV = NFEV + N
...
C
      END
*DECK DFDJC3
      SUBROUTINE DFDJC3 (M, N, X, FVEC, FJAC, LDFJAC, IFLAG,
     +   EPSFCN, WA)
c$$$      SUBROUTINE DFDJC3 ( M, N, X, FVEC, FJAC, LDFJAC, IFLAG,
c$$$     +   EPSFCN, WA)
...
C     The subroutine statement is
C
C       SUBROUTINE DFDJC3(M,N,X,FVEC,FJAC,LDFJAC,IFLAG,EPSFCN,WA)
C
C     where
C
