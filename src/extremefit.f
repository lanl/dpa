...
C 
	CHARACTER*100 DATAFILE,flag_showfit,file_fitting
	INTEGER NDATA,NSEC,M,N           !M: number of data  N: number of variables
	...
	!read(*,*) datafile,nsec,cutperct
	!datafile='o.dat'	!o1a1e.dpa'
	!nsec=100
	!cutperct=0.96
...
!	write(*,999) cutdpa,SERR,RCORR(M,X,FVEC),(X(i),i=1,2)
	...
	STOP
 999	FORMAT (2x,F10.5,1x,F12.7,1x,F10.7,3x,'DPA CUT & SERROR & RERROR',/
...
c$$$	enddo
 500	format(1x,I5,1x,F10.6,1x,F10.6,2x,F15.6,10x,"I  X(I)  Y(I)  SUM")
	...
	subroutine ini_data(datafile,ndata)
	implicit none
	character*100 datafile
	integer ndata,ndata0,i
	parameter (ndata0=10000)
	real*8  y(ndata0),tmp
	common /DATA0/ Y

	open (unit=20,file=datafile,status='old')
	ndata=0
	do while (ndata .ge. 0) 
	   read(20,*,end=100,err=100)  tmp
	   ndata=ndata+1
	   Y(ndata)=tmp
	enddo
 100	close(20)
...
!	write(*,*) 'CUTX1>  ', cutx1,datamin,delta
	...
	open(unit=21,file=file_output,status='unknown')
	write(21,'(A1,A11,2x,A12,2x,A12,2x,A12)'),"#","X(i)","Y(i)",
     &       "Fitted_Y(i)","DELTA"
	do i=1,ntot
	   fitted_value=extremeval(x1(i),x(1),x(2))
!	   write(*,*) 'CHECK> ', x1(i),y1(i),fitted_value
!	   write(21,'(A,4(F12.7,2x))') 'CHECK> ', x1(i),y1(i),fvec(i),fitted_value
	   write(21,'(4(F12.7,2x))') x1(i),y1(i),fitted_value,fvec(i)
	enddo
	write(*,'(A)') 'Fitting results see: extremefittingresult.dat'
