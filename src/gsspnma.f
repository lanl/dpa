...
c
	call gene_surfdpa(calc_info,infocalc,ev_threshold,file_cacrd,file_surfp,file_dpa,nca,nsurf,cutcc,cutlg,cutll,wcc,wctc,wlg,wll,time1,time2)
...
c       
 	...
 	subroutine gene_surfdpa(calc_info,infocalc,ev_threshold,file_cacrd,file_surfp,file_dpa,nca,nsurf,cutcc,cutlg,cutll,wcc,wctc,wlg,wll,time1,time2)
	implicitnone
...
c
	...
	close(20)
c
	call hessian_apoprotein(calc_info,infocalc,cacrd,nca,nca3,ndimca,cutcc,wcc,wctc,ev_threshold)
	...
	do i=1,nsurf
	   ...
	   call DPA_1th_perturb(calc_info,infocalc,cacrd,hetcrd,nca+nhet,nca3,nhet3,ndimca,ndimhet,cutlg,cutll,wlg,wll,pert)
	   if(calc_info.ne.0) return
...
c
      subroutine CharmmDiagq(N,NDD,mode_need,DD,ev,vec)
      implicit none
...
c
      ...
      call DIAGQ(N,mode_need,DD,VEC,A,B,P,W,EV,TA,TB,Y,NADD)
c
      return
      end
c
...
c      INTEGER   NAT0,N_DIM0
c      PARAMETER (NAT0=1200, N_DIM0=(9*NAT0*NAT0+3*NAT0)/2)
      Real*8  DD(*) !DD(N_DIM0)
C 
...
c*Desk :Hessian Matrix :Begin!
	subroutine hessian_apoprotein(calc_info,infocalc,cacrd,nca,nca3,ndimca,cutcc,wcc,wctc,ev_threshold)
	implicit none
	...
	integer   nat0,nat30,ione
	parameter (nat0=1200,nat30=3*nat0,ione=1)
	real*8  ev_threshold,ev0(nat30),vec0(nat30*nat30)
	common  /MODE0/ ev0,vec0
c
	if(nca.gt.nat0) then
...
c     
	...
	enddo
c
	call CharmmDiagq(nca3,ione,nca3,hessian,ev0,vec0)
...
c*Desk :Hessian Matrix :Begin!
	subroutine DPA_1th_perturb(calc_info,infocalc,cacrd,hetcrd,nsys,nca3,nhet3,ndimca,ndimhet,cutlg,cutll,wlg,wll,lgpert)
	implicit none
	...
	integer   nat0,nat30,nat1,nat31,ione !nat1: maximum number of atoms that ligand binds
	parameter (nat0=1200,nat30=3*nat0,nat1=100,nat31=3*nat1,ione=1)
	real*8    ev0(nat30),vec0(nat30*nat30),ev(nca3)
	integer   rp(nca3),col_id(nca3,nat31),h_id(nca3,nat31),idgnz(nca3)
	real*8    pessian(nca3*nat31),hv(nca3),lgpert
	common   /MODE0/ ev0,vec0
        integer   i,j,k,l,i0,j0,i1,j1,ki0,kj0,kh
c
        nca=nca3/3
        nhet=nhet3/3
c
...
c       
!Diagonalize Kessian
	call CharmmDiagq(nhet3,ione,nhet3,kessian,kev,kvec)
...
c
	kh=0
        do i=1,nca3		!pessian(i,i)=0 if ith atom do NOT inter with Ligands
		 ...
		 enddo    
		 if(j.le.(i+mod(3-mod(i,3),3))) then !Diagonal Term: from the sum_i_k=0 requirement of Hessian
		    i0=mod(j,3)
		    if(i0.eq.0) i0=3
		    do j0=i0,nhet3,3 
		 ...
		 if(j.ne.i)  then
		    ...
		    h_id(j,rp(j))=kh !in row j, nonzero element's value is the kh element of New Hessian
		    col_id(j,rp(j))=i !in row j,column index of NONZERO element is i in origianl Hessian
		 endif
	      endif
           enddo
        enddo
c
	...
	do i=1,nca3
	   do k=1,nca3
	      tmp=0.
	      do j=1,rp(k)
		 tmp=tmp+pessian(h_id(k,j))*vec0(i0+col_id(k,j)) !Sum_j {P(k,j)*V(j,i)}
	      enddo
	      hv(k)=tmp		!H(k,i)
	   enddo
	   tmp=0.
